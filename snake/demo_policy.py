...
snake = Snake(rewards=rewards, verbose=False)

# initialize the model
n_input = 2 * snake.grid_size * snake.grid_size
model = FullyConnected(n_input=n_input, n_hidden=200, n_classes=4)
# model = ConvNet(n_frames=2, n_classes=4, n_blocks=1)

start_time = time.time()
# train(model, snake, batch_size=100, n_iterations=500,
#        gamma=0.95, learning_rate=0.001, plot=True)
end_time = time.time()
print('Elapsed %.2f seconds' % (end_time - start_time))

test(model, snake, save=True)
